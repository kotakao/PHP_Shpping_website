...
<div class="all_commodity">
	  <div class="detailed">
		  <form action="account_control.php" method="post" id="form1" name="form1">
		  <p><div class="del" style="float: none;border-style: none;">
			  刪除帳號：<input type="text"  name='del_account' required/ >
		...
		if(isset($_POST['del_account']))
	{
		...
		$sql_acc="'".$acc."'";
		$sql_del="DELETE FROM `account` WHERE `account`.`account` =$sql_acc";//sql指令
	...
	<div class="name">性別</div>
		  <div class="account">E-mail</div>
	<div class="name">電話</div><br>
<?php 
	  	$sql_query="SELECT account FROM `account`";//sql指令
		$result=mysqli_query($db_link,$sql_query);//指定資料庫連結,打進去的指令
		...
		while($row_result=mysqli_fetch_row($result))
					{
						foreach($row_result as $item => $value)
						{
							...
							$acc_value="'".$value."'";
							$sql_NAME="SELECT name FROM `account` WHERE account = $acc_value";//sql指令　姓名
							...
							$sql_gender="SELECT gender FROM `account` WHERE account = $acc_value";//sql指令　
							$result_gender=mysqli_query($db_link,$sql_gender);//指定資料庫連結,打進去的指令
					
							while($row_result_gender=mysqli_fetch_row($result_gender))
								{
								foreach($row_result_gender as $item_gender => $gender)
									{
									echo "<div class=".  "'"."name"."'"  .">".$gender."</div>";
									}
								}
							
							$sql_EMAIL="SELECT Email FROM `account` WHERE account = $acc_value";//sql指令　信箱
							...
							while($row_result_EMAIL=mysqli_fetch_row($result_EMAIL))
								...
								foreach($row_result_EMAIL as $item_EMAIL => $EMAIL)
									{
									echo "<div class=".  "'"."account"."'"  .">".$EMAIL."</div>";
									}
								}
							
							$sql_phone="SELECT phone FROM `account` WHERE account = $acc_value";//sql指令　
							...
							while($row_result_phone=mysqli_fetch_row($result_phone))
								...
								foreach($row_result_phone as $item_phone => $phone)
									{
									echo "<div class=".  "'"."name"."'"  .">".$phone."</div><br>";
									}
								}
						}
					}
