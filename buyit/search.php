...
<div class="home">
  ...
  <div class="all_commodity">
    <form action="search.php" method="post" id="form2" name="form2">
      搜索商品名稱：
      <input type="text" name='searchgo' autocomplete="off" list="searchlist" id='searchgo' />
      <datalist id="searchlist"></datalist>
      <input type="hidden" id="hid" value="1">
      <input type="hidden" id="hid2" name="hid2" value="a4444">
      ...
      <script>
		...
		$.ajax({
			...
			console.log(ss);
			var op = new Array(searchlist.length);
			if(searchlist.length>5){
				list_length=5;
			}else if(searchlist.length<=5){
				list_length=searchlist.length;
			}
		  for (var j = 0; j < list_length; j++) {
			  var lists = searchlist[j];
			  op[j] = document.createElement("option");
			  console.log(op);
			  console.log(ss);
			  op[j].setAttribute("label", "");
			  op[j].setAttribute("value", lists.value);
			 // console.log(ss.childNodes.length);
			 // console.log(scount);
			  ss.appendChild(op[j]);
		  }
    ...
    if ( isset( $_POST[ 'searchgo' ] ) ) {
      ...
      while ( $row_result_ID = mysqli_fetch_row( $result_ID ) ) {
        foreach ( $row_result_ID as $item_ID => $ID ) {
          ...
          ?>
    <input type="image" <?php echo "img src=image/".$ID.".jpg";?> width='200px' height='200px'/>
    <br>
    <?php
    ...
    <input type="button"  class="text_commodity" <?php echo "value=".$NAME;?> style="border:0;cursor:pointer" />
    <br>
    <?php
    }
    }
    ...
    while ( $row_result_price = mysqli_fetch_row( $result_price ) ) {
      foreach ( $row_result_price as $item_price => $price ) {
        ?>
    <input type="button"  class="text_commodity2" <?php echo "value=NT$".$price;?> style="border:0;cursor:pointer" />
    <br>
    <?php
    }
    }
    echo "</div>";
    echo "</form>";
    //echo "<div class=".  "'"."searchgo"."'"  .">".$NAME.""."</div>";
    }
    }
    }
    if ( $v == 0 ) {
