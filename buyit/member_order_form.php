...
<div class="all_commodity">
	...
	<br>
<?php 
	  	$sql_query="SELECT ID FROM `order_form` WHERE `order_account` = $sql_member";//sql指令
	  ...
	  if(isset($_POST['del_account']))//查詢訂單
	{
		$acc=$_POST['del_account'];
		
		   $sql_cheak="SELECT order_id FROM `order_form` WHERE `order_account` = $sql_member";//sql指令 帳號
			$result_cheak=mysqli_query($db_link,$sql_cheak);//指定資料庫連結,打進去的指令
				while($row_result_cheak=mysqli_fetch_row($result_cheak))//帳號確認
				{
					foreach($row_result_cheak as $item_cheak => $cheak)
					{
						if($cheak==$acc)
						{
							$cheak;
